% fitness function
function [bestpa,fit]=fitness(par,SAI)
...
for i=2:n    %  changed the range already
    ...
    if r<tan(angle)    % check the range of value r
        fsc=0;
    else
        fsc=-1;
    end
...
w1=1/4; % for fitness function= w1+w2+w3=1
w2=1/4;
w3=2/4;
obv=-w1*fec-w2*ffr+w3*fsai;
ht=fec+TC+FAC+TAC+FSC;

%fitness function value
fit=obv+TC+FAC+TAC+FSC;
